...
GENERATED_ANGLE_PATH ="../structure_model/data/output.pkl"
...
def load_generated_angles(file_path)->pd.DataFrame:
    with open(file_path, "rb") as f:
        angles = pickle.load(f)
    angles = torch.Tensor(np.array([
        np.pad(
            a, 
            ( (0, CONFIG["max_seq_len"]-a.shape[0]), (0,0) ),
            mode="constant",
            constant_values=0
        ) for a in angles
    ]))
    chunckified_angles = [angles[i:i+CONFIG["batch_size"]] for i in range(0, len(angles), CONFIG["batch_size"])]
    return chunckified_angles
...
@torch.no_grad
def denoise(batch, generated_angles, model:PeptideDiff, noise_schedule, transition, diverse):
    ...
    ligand_angles = generated_angles.to(DEVICE)# <--- !!! REPLACE THIS !!!
...
if __name__ == "__main__":
    ...
    generated_angles = load_generated_angles(GENERATED_ANGLE_PATH)
    ...
    for idx, batch in enumerate(testing_dataloader):
        ...
        ids, true_seq, pred_seq, rec_rates = denoise(
            batch, generated_angles[idx], model, 
            noise_schedule, transition, True
        )

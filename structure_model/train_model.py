...
# from dataset import AnglesDataset, NoisedAnglesDataset
...
def get_dataloader(file_path):
    ...
    train_angle_ds = NoisedAnglesDataset(
        train_angle_ds,
    ...
    val_angle_ds = NoisedAnglesDataset(
        val_angle_ds,
        timesteps=CONFIG["timesteps"],
    )
...
def train_model(encoder_config:BertConfig, decoder_config:BertConfig, train_dataloader: DataLoader, val_dataloader: DataLoader):
    ...
    model = ConditionalBertForDiffusion(
        ...
        ]*4 + [ConditionalBertForDiffusion.angle_loss_func]*4,
    )
